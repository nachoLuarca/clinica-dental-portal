    ...
    return (
      <div className="flex flex-col gap-3 rounded-2xl bg-secondary/60 p-5">
        ...
        <p className="text-sm text-secondary-foreground/80">
          Ocurrió un problema al consultar el equipo. Contáctala directamente
          para coordinar tu hora, o intenta de nuevo más tarde.
        </p>
      </div>
    )
  }

  // Sin profesionales activos igual se puede reservar en modo "cualquiera
  // disponible": la API resuelve internamente quién cubre el horario.
  if (!profesionales || profesionales.length === 0) {
    return (
      <button
        type="button"
        onClick={onElegirCualquiera}
        className="group flex w-full items-center gap-3 rounded-2xl bg-accent/10 p-4 text-left ring-1 ring-accent/30 transition-all duration-200 hover:-translate-y-0.5 hover:ring-2 hover:ring-accent hover:shadow-md"
      >
